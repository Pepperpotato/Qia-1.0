...
class User(models.Model):
    ...
    reg_time = models.DateTimeField(default=datetime.now)   # 注册时间
    birthday = models.CharField(max_length=20, null=True)   # 生日
    user_photo = models.CharField(max_length=100, default='/admin/upload/avatar_blank.gif')    # 用户头像
    user_status = models.IntegerField(default=0)     # 用户状态 0为正常,1为锁定
    vip_level = models.IntegerField(default=0)   # 会员等级
    safety_grade = models.IntegerField(default=0)     # 安全积分
...
class User_grade(models.Model):
    ...
    change_source = models.CharField(max_length=50, null=True)     # 积分变更来源
    change_number = models.IntegerField(null=True)     # 积分变更数值
    change_time = models.DateTimeField(default=datetime.now, null=True)     # 变更时间
    changed_grade = models.IntegerField(null=True, default=0)     # 变更后积分
    growth_value = models.IntegerField(null=True, default=0)      # 会员成长值
...
class User_address(models.Model):
    ...
    location = models.CharField(max_length=20, null=True)  # 所在地
...
class User_collection(models.Model):
    ...
    collection_time = models.DateTimeField(default=datetime.now)    # 收藏时间
...
class User_notice(models.Model):
    ...
    sendtime = models.DateTimeField(default=datetime.now)    # 发送时间
...
class User_offer(models.Model):
    offer_content = models.CharField(max_length=200, null=True)    # 优惠内容描述
    offer_time = models.DateTimeField(null=True)    # 优惠时间
    # status = models.IntegerField(default=0)   # 使用状态 0为未使用，1为使用
    offer_door = models.IntegerField(null=True)   # 优惠门槛
    offer_money = models.IntegerField(null=True)   # 优惠金额
